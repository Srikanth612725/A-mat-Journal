"""
CELL 6A: TIER 1 WITH CORRECT MPM API
=====================================

This uses the ACTUAL mpm_optimized.py API:
    MPM2D_Optimized(domain_x, domain_y, nx, ny, su, E, nu, rho, use_gimp=True)
    add_soil_block(x_range, y_range, ppc=4)
    add_strip_foundation(center_x, y_base, width, thickness, density)

Features:
✅ Correct API (no more errors!)
✅ All 5 standard methods calculated
✅ Google Drive backup (survives disconnects)
✅ Checkpoint/resume capability
✅ Auto-downloads backups
"""
...
for idx_orig, row in df_to_run.iterrows():
    ...
    try:
        ...
        # ✅ CORRECT API: domain_x, domain_y are tuples!
        mpm = MPM2D_Optimized(
            ...
            ny=int(row['ny']),
            su=su,
            E=E,  # Pass E directly, not E_ratio!
            nu=nu,
            rho=rho,
            use_gimp=use_gimp
        )
        ...
        mpm.add_strip_foundation(
            ...
            density=foundation_density
        )

        print(f"   ✓ Soil block: 0-{domain_width}m × 0-{soil_depth}m")
        print(f"   ✓ Foundation: {width}m × {foundation_thickness}m @ x={center_x}m, y={y_base}m")
        ...
        # ✅ FIX: Set foundation velocity using the correct API
        mpm.set_foundation_velocity(0, -rate)  # (vx=0, vy=-rate, negative = downward)
        ...
        # Brinch Hansen
        try:
            result = brinch_hansen_80_method(settlements, loads)
            if result:
                Q_hansen = result['Q_ult']
        except:
            pass

        # Fuller-Hoy
        try:
            result = fuller_hoy_method(settlements, loads)
            if result:
                Q_fuller = result['Q_ult']
        except:
            pass

        # 0.1B
        try:
            result = settlement_10_percent_method(settlements, loads, width)
            if result and result['Q_ult'] is not None:
                Q_10pct = result['Q_ult']
        except:
            pass
        ...
        print(f"   ✓ Chin-Konder:   {Q_chin:.0f} kN/m" if not np.isnan(Q_chin) else "   ✗ Chin-Konder:   Failed")
        print(f"   ✓ Brinch Hansen: {Q_hansen:.0f} kN/m" if not np.isnan(Q_hansen) else "   ✗ Brinch Hansen: Failed")
        print(f"   ✓ Fuller-Hoy:    {Q_fuller:.0f} kN/m" if not np.isnan(Q_fuller) else "   ✗ Fuller-Hoy:    Failed")
        print(f"   ✓ 0.1B:          {Q_10pct:.0f} kN/m" if not np.isnan(Q_10pct) else "   ✗ 0.1B:          Failed")
        ...
        if len(all_results) % 3 == 0 or len(all_results) == total_runs:
            try:
                ...
                print(f"   ⚠️  Download blocked (data safe in Drive)")
        else:
...
if len(successful) > 0:
    ...
    print(df_final[available_cols].head(12).to_string(index=False))
    ...
    for method in ['davisson', 'chin', 'hansen']:
        ...
        if error_col in successful.columns:
            avg_error = successful[error_col].mean()
            std_error = successful[error_col].std()
...
2. 💾 Downloads: tier1_results_FINAL.csv

Next: Compare all 5 methods using the comparison cell!
""")
